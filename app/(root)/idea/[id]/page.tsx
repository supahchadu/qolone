...
const Page = async ({ params } : { params: Promise<{ id: string}>}) => {
    ...
    const [post, { select: editorPosts }] = await Promise.all([
        ...
        client.fetch(PLAYLIST_BY_SLUG_QUERY, {slug: "thought-provoking"}),
    ]);
        ...
        <section className="section_container">
            ...
            <div className="space-y-5 mt-10 max-w-4x mx-auto">
                <div className="flex-between gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-26-medium !text-black-300">@{post.author.user}</p>
                    </div>
                    </Link>
                ...
                {parsedContent ? (
                    ...
                    <p className="no-result"> No Additional Details Provided</p>
                )}
            </div>
            ...
            {editorPosts?.length > 0 && (
                <div className="max-w-4xl mx-auto">
                    <p className="text-30-semibold">Thought Provoking</p>
                    <ul className="mt-7 card_grid-sm">
                        {editorPosts?.map((post:IdeaCardType, index: number)=>(
